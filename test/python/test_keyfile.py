...
class TestKeyFileMaterial(unittest.TestCase):
    ...
    def test_hmac_secret_envelope_is_unwrapped(self):
        ...
        self.assertEqual(_load_key_material(path, "sign", "HMAC-SHA256"), secret)
        self.assertEqual(_load_key_material(path, "verify", "HMAC-SHA256"), secret)

    def test_ed25519_envelope_role_aware_fields(self):
        kp = ProofSigner.generate_key_pair()
        path = self._write(kp)
        self.assertEqual(_load_key_material(path, "sign", "Ed25519"), kp["privateKey"])
        self.assertEqual(_load_key_material(path, "verify", "Ed25519"), kp["publicKey"])
    ...
    def test_plain_text_key_file_is_used_verbatim(self):
        ...
        self.addCleanup(os.remove, path)
        self.assertEqual(_load_key_material(path, "sign", "HMAC-SHA256"), "my-plain-text-secret")
