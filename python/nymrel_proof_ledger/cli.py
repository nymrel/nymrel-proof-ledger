...
def main(argv=None):
    ...
    keygen_parser.add_argument("--algo", default="HMAC-SHA256", choices=["HMAC-SHA256", "Ed25519"], help="Algorithm")
    keygen_parser.add_argument("--out", help="Output file path")

    # attest
    attest_parser = subparsers.add_parser("attest", help="Generate cryptographic attestation")
    ...
    attest_parser.add_argument("--signer", default="nymrel-agent", help="Signer identity")
    attest_parser.add_argument("--algo", default="HMAC-SHA256", choices=["HMAC-SHA256", "Ed25519"], help="Algorithm")
    attest_parser.add_argument("--status", default="SUCCESS", choices=["SUCCESS", "FAILURE", "ATTESTED"], help="Status")
    attest_parser.add_argument("--out", default="proof.json", help="Output proof receipt JSON file")
    ...
    verify_parser.add_argument("proof", nargs="?", default="proof.json", help="Proof receipt JSON path")
    ...
    verify_parser.add_argument("--check-files", action="store_true", help="Verify disk file hashes")
    verify_parser.add_argument("--json", action="store_true", help="Output verification result as JSON")

    # badge
    badge_parser = subparsers.add_parser("badge", help="Generate SVG badge from proof.json")
    badge_parser.add_argument("proof", nargs="?", default="proof.json", help="Proof receipt JSON path")
    badge_parser.add_argument("--out", default="badge.svg", help="Output file path")
    badge_parser.add_argument("--format", default="svg", choices=["svg", "shield", "html"], help="Output format")
    badge_parser.add_argument("--compact", action="store_true", help="Compact SVG dimensions")

    # inspect
    inspect_parser = subparsers.add_parser("inspect", help="Display human-readable proof audit trail")
    inspect_parser.add_argument("proof", nargs="?", default="proof.json", help="Proof receipt JSON path")

    # export
    export_parser = subparsers.add_parser("export", help="Export proof receipt")
    export_parser.add_argument("proof", nargs="?", default="proof.json", help="Proof receipt JSON path")
    export_parser.add_argument("--format", default="markdown", choices=["markdown", "jsonld", "html"], help="Export format")
    ...
    if args.command == "keygen":
        if args.algo == "Ed25519":
            ...
            out_str = json.dumps({"algorithm": "HMAC-SHA256", "secretKey": sec}, indent=2)
    ...
    if args.command == "attest":
        ...
        file_list = []
        if args.files:
            file_list = [{"path": p.strip()} for p in args.files.split(",") if p.strip()]
    ...
    if args.command == "verify":
        ...
        if args.check_files:
            print(f"Disk Files:  {'ALL MATCHED' if result['artifactsValid'] else 'TAMPERED/MISSING'} ({result['checkedArtifacts']} checked)")
        ...
        print(f"\nOverall: {'PASSED (TRUSTED)' if result['valid'] else 'FAILED (UNVERIFIED)'}\n")
        sys.exit(0 if result["valid"] else 1)
    ...
    if args.command == "inspect":
        ...
        print(f"Task:        {receipt.get('task', {}).get('name')} ({receipt.get('task', {}).get('status')})")
        print(f"Merkle Root: {receipt.get('merkle', {}).get('root')}")
        print(f"Signer:      {receipt.get('signature', {}).get('signerIdentity')} [{receipt.get('signature', {}).get('algorithm')}]")
        print(f"Artifacts:   {len(receipt.get('artifacts', []))}")
        for idx, art in enumerate(receipt.get("artifacts", [])):
            print(f"  [{idx+1}] {art.get('path')} -> SHA256: {art.get('sha256')} ({art.get('sizeBytes')} B)")
        print("")
        sys.exit(0)
        ...
        if args.format == "html":
            ...
            out_str = json.dumps({
                ...
                "parentOrganization": {
                    "@type": "Organization",
                    "name": "Nymrel",
                    "parentOrganization": {"@type": "Organization", "name": "JalenBuilds LLC"}
                },
